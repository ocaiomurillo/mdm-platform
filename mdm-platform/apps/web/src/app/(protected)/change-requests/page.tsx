...
export default function ChangeRequestPage() {
  ...
  const [partnerIdInput, setPartnerIdInput] = useState(initialPartner);
  const [selectedPartnerId, setSelectedPartnerId] = useState(initialPartner);
  ...
  const [bulkInput, setBulkInput] = useState(initialPartner);
  ...
  const parsedBulkPartnerIds = useMemo(
    () => parseBulkInput(bulkInput, mode === "massa" ? selectedPartnerId : undefined),
    [bulkInput, mode, selectedPartnerId]
  );

  useEffect(() => {
    const params = new URLSearchParams();
    if (selectedPartnerId) {
      params.set("partner", selectedPartnerId);
    }
    ...
    router.replace(`/change-requests${query ? `?${query}` : ""}`, { scroll: false });
  }, [router, selectedPartnerId, statusFilter, typeFilter]);
  ...
  const loadPartner = useCallback(async () => {
    ...
    setPartnerError(null);
    try {
      const response = await axios.get(`${apiUrl}/partners/${selectedPartnerId}/details`, {
  ...
  }, [apiUrl, selectedPartnerId, router]);

  const loadChangeRequests = useCallback(async () => {
    if (!selectedPartnerId || !apiUrl) {
      setRequests([]);
      return;
    }
    const token = typeof window !== "undefined" ? localStorage.getItem("mdmToken") : null;
    ...
    try {
      ...
      const response = await axios.get(
        `${apiUrl}/partners/${selectedPartnerId}/change-requests?${params.toString()}`,
        {
          headers: { Authorization: `Bearer ${token}` }
        }
      );
  ...
  }, [apiUrl, selectedPartnerId, statusFilter, typeFilter, router]);
  ...
  useEffect(() => {
    ...
    setBulkInput(mode === "massa" && selectedPartnerId ? selectedPartnerId : "");
  }, [selectedPartnerId]);
  ...
  useEffect(() => {
    ...
    setSubmitSuccess(null);
    setSubmitResult(null);
    if (mode === "massa" && !bulkInput.trim() && selectedPartnerId) {
      setBulkInput(selectedPartnerId);
    }
  }, [mode, selectedPartnerId, bulkInput]);
  ...
  const handleUpdateStatus = async (request: ChangeRequestItem, newStatus: string) => {
    ...
    if (!apiUrl) {
      setActionError("URL da API não configurada.");
      return;
    }
    const token = typeof window !== "undefined" ? localStorage.getItem("mdmToken") : null;
    if (!token) {
    ...
    try {
      await axios.patch(
        `${apiUrl}/partners/${request.partnerId}/change-requests/${request.id}`,
        { status: newStatus },
        {
          headers: { Authorization: `Bearer ${token}` }
        }
      ...
      const message = error?.response?.data?.message;
      setActionError(typeof message === "string" ? message : "Não foi possível atualizar o status.");
    } finally {
      setActionLoadingId(null);
    }
  };

  const handleDeleteRequest = async (request: ChangeRequestItem) => {
    if (!apiUrl) {
      setActionError("URL da API não configurada.");
      return;
    }
    const confirmed = window.confirm("Tem certeza que deseja remover esta solicitação?");
    if (!confirmed) {
      return;
    }
    ...
    try {
      await axios.delete(`${apiUrl}/partners/${request.partnerId}/change-requests/${request.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setActionMessage("Solicitação removida com sucesso.");
      await loadChangeRequests();
    } catch (error: any) {
      if (error?.response?.status === 401) {
        localStorage.removeItem("mdmToken");
      ...
      const message = error?.response?.data?.message;
      setActionError(typeof message === "string" ? message : "Não foi possível remover a solicitação.");
    } finally {
      setActionLoadingId(null);
    }
  };
    ...
    try {
      if (mode === "individual") {
        const response = await axios.post(
          `${apiUrl}/partners/${selectedPartnerId}/change-requests`,
          {
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
        <div className="flex flex-col gap-4 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-sm text-zinc-500">Gerencie, crie e acompanhe solicitações de alteração para parceiros.</p>
          </div>
          <form className="flex flex-col gap-4" onSubmit={handleFilterSubmit}>
            <div className="grid gap-4 md:grid-cols-4">
              <div className="md:col-span-2 flex flex-col gap-2">
                <label htmlFor="partner-filter" className="text-sm font-medium text-zinc-700">
                  Parceiro (ID)
                </label>
                <input
                  id="partner-filter"
                  ...
                  placeholder="Informe o ID do parceiro"
                  className="rounded-lg border border-zinc-200 px-3 py-2 text-sm text-zinc-700 focus:border-zinc-900 focus:outline-none"
                />
              </div>
              ...
              <div className="flex flex-col gap-2">
                <label htmlFor="type-filter" className="text-sm font-medium text-zinc-700">
                  Tipo
                </label>
                <select
                  id="type-filter"
          ...
          {partnerLoading ? (
            <p className="text-sm text-zinc-500">Carregando dados do parceiro selecionado...</p>
          ) : partnerError ? (
            ...
            <div className="flex flex-wrap gap-2 text-xs text-zinc-600">
              <span className="rounded-full bg-zinc-100 px-3 py-1">{partnerSummary.nome_legal}</span>
              <span className="rounded-full bg-zinc-100 px-3 py-1">Documento: {partnerSummary.documento}</span>
            ...
            <p className="text-sm text-zinc-500">Não encontramos dados para o parceiro informado.</p>
          ) : null}
        </div>
        ...
        {selectedPartnerId ? (
          <div className="flex flex-col gap-4 rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm">
            {partnerLoading ? (
              <p className="text-sm text-zinc-500">Carregando informações do parceiro...</p>
            ) : !partnerSummary ? (
              <p className="text-sm text-zinc-500">Informe um parceiro válido para gerenciar solicitações.</p>
            ) : (
              <>
                <div className="flex flex-col gap-1">
                  <h2 className="text-xl font-semibold text-zinc-900">{partnerSummary.nome_legal}</h2>
                  <p className="text-sm text-zinc-500">Documento: {partnerSummary.documento}</p>
                </div>
                <div className="flex flex-wrap items-center gap-3 text-sm">
                  ...
                  <div className="flex flex-wrap gap-2 text-xs text-zinc-600">
                    <span className="rounded-full bg-zinc-100 px-3 py-1">Status atual: {partnerSummary.status ?? "-"}</span>
                  </div>
                </div>
                ...
                {activeTab === "existing" ? (
                  <div className="flex flex-col gap-4">
                    {actionError ? (
                      <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-600">{actionError}</div>
                    ) : null}
                    {actionMessage ? (
                      <div className="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">{actionMessage}</div>
                    ) : null}
                    {requestsError ? (
                    ...
                    {!requestsLoading && !requests.length ? (
                      <p className="text-sm text-zinc-500">Nenhuma solicitação encontrada com os filtros atuais.</p>
                    ) : null}
                      ...
                      {requests.map((request) => (
                        <div key={request.id} className="flex flex-col gap-3 rounded-xl border border-zinc-200 p-4">
                          ...
                          <div className="flex flex-wrap items-center gap-3 text-xs text-zinc-600">
                            <label htmlFor={`status-${request.id}`} className="font-semibold text-zinc-700">
                              Atualizar status
                            </label>
                            <select
                              id={`status-${request.id}`}
                              value={request.status}
                              onChange={(event) => handleUpdateStatus(request, event.target.value)}
          ...
          <div className="rounded-2xl border border-dashed border-zinc-300 bg-white p-6 text-center text-sm text-zinc-500">
            Informe um parceiro para visualizar e criar solicitações de mudança.
          </div>
        )}
      </div>
    </main>
